    ...
    *,
    ...
    Args:
        ...
        init_args: Intialization arguments provided by
            `init_convert_IC600_to_ome_zarr`.
    """
    ...
    patterns = [f"*{init_args.well_ID[0]} - {init_args.well_ID[1:]}"
    ...
    for i_c, wavelength_id in enumerate(wavelength_ids):
        patterns = [f"*{init_args.well_ID[0]} - {init_args.well_ID[1:]}(*{wavelength_id}*."
                    f"{init_args.image_extension}"]
        if init_args.image_glob_patterns:
            patterns.extend(init_args.image_glob_patterns)
        ...
        if len(filenames) == 0:
            raise ValueError(
                "Error in convert_IC6000_to_ome_zarr: len(filenames)=0.\n"
...
if __name__ == "__main__":
    ...
    run_fractal_task(
        task_function=convert_IC6000_to_ome_zarr,
