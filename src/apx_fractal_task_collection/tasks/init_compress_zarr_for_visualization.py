...
# Zurich.
"""
Initializes the parallelization list for the Compress OME-Zarr for
Visualzation task.
"""
import logging
...
@validate_call
def init_compress_zarr_for_visualization(
        *,
        # Fractal parameters
        zarr_urls: list[str],
        zarr_dir: str,
    ...
    used to perform Compress OME-Zarr for Visualzation task.
    ...
    Args:
        ...
        zarr_dir: path of the directory where the new OME-Zarrs will be
            ...
            (standard argument for Fractal tasks, managed by Fractal server).
        channel_label: Label of the channel in which blobs will be detected.
        output_label_image_name: Name of the zarr image that will
            contain the label image with detected blob centroids.
            Defaults to "0". In case you saved, for example,
            illumination corrected images in the same zarr without overwriting
             the original images, you can  specify a different
             name here (e.g. "0_illum_corrected").
    ...
    """
    logger.info(
        f"Running `init_detect_blob_centroids` for {zarr_urls=}"
    )
...
if __name__ == "__main__":
    ...
    run_fractal_task(
        task_function=init_compress_zarr_for_visualization,
